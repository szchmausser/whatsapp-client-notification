...
2. Escuchar en tiempo real los mensajes de chats de WhatsApp.
3. Persistir todo en una base de datos **MySQL**.

Este servicio es un **"data collector"**, no un producto final. Otros sistemas (procesadores de IA, APIs, dashboards) van a consumir la base de datos MySQL directamente. Este proyecto **no** implementa esos consumidores.
...
- **Usar MySQL como base de datos** para soportar acceso concurrente desde múltiples procesos (collector + Astro web UI).
- **Consultas SQL directas** sin ORM adicional para mantener la filosofía KISS.
...
- Por ahora, el servicio se enfoca en **un único chat específico**, identificado por su JID y configurable (por ejemplo, vía variable de entorno). No se requiere en esta fase descubrir ni monitorear múltiples chats dinámicamente.
- Escuchar los eventos de mensajes entrantes correspondientes a ese chat y reenviar (persistir) cada mensaje que llegue hacia la base de datos MySQL.
...
- El diseño debe dejar la puerta abierta a soportar más de un chat en una iteración futura (p. ej., que el JID no quede hardcodeado en la lógica de negocio), pero no es necesario construir esa generalización ahora si añade complejidad significativa.
...
- El servicio debe poder determinar, en todo momento, **cuál es el último mensaje que tiene guardado** para el chat monitoreado (por timestamp y/o id de mensaje).
...
### 3.4 Identificación del chat monitoreado
- Mantener un registro simple del chat configurado (JID, y su metadata básica si está disponible: nombre, descripción).
- El servicio asume que la cuenta de WhatsApp usada ya tiene acceso a ese chat.

### 3.5 Persistencia
- Toda la información debe quedar en una base de datos MySQL, estructurada de forma que sistemas externos puedan leerla directamente (sin pasar por una API de este servicio).
- El esquema debe cubrir como mínimo: el/los chat(s) monitoreados, mensajes, y estado de sincronización (último mensaje conocido) por chat. El diseño exacto de columnas queda a criterio del agente, siguiendo el principio de guardar el payload crudo del mensaje además de las columnas normalizadas que se necesiten consultar directamente.

## 4. Fuera de alcance (explícitamente, no ambigüedad permitida)

El agente **no debe** implementar nada de lo siguiente en esta fase, aunque parezca una extensión natural. El objetivo de esta primera versión es exclusivamente tener la base de datos local bien sincronizada con el historial y los mensajes nuevos del chat configurado:
...
1. En qué evento(s) de Baileys llegan realmente los mensajes del chat configurado en la versión instalada.
...
3. Qué requiere la cuenta de WhatsApp usada para poder recibir mensajes de ese chat.
...
- Con el chat configurado, los mensajes nuevos se capturan en tiempo real y quedan en MySQL sin duplicados.
...
- El esquema de la base de datos está definido en MySQL, y un tercero (otro proceso) puede conectarse y leer los datos sin necesitar conocer detalles internos del servicio.
...
- Diseño exacto de columnas y tipos en el esquema MySQL.
